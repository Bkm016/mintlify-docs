...
Fluxon 官方 VS Code 扩展为 `.fs` 文件与 YAML 嵌入块带来语法高亮、智能补全、注解/片段模板与函数目录同步能力。本章总结其安装方式、常用配置及维护流程，便于团队快速统一开发体验。

## 功能概览

- **语法高亮**：覆盖关键字、变量引用、运算符与注解块。
- **智能补全**：关键字、系统函数、扩展函数、链式 `::` 调用及当前工作区的 `def`/`fun` 块均可被建议，并附带参数占位符。
...
| `Toggle Line Comment` (`Ctrl+/`) | 针对 Fluxon 语法插入 `#` 注释。 |
| `Toggle Block Comment` (`Shift+Alt+A`) | 插入 `/* ... */` 区块注释，支持多光标。 |
...
| 键 | 默认值 | 作用 |
| --- | --- | --- |
| `fluxonCompletion.catalogPath` | *(空)* | 自定义函数目录路径（如 `${workspaceFolder}/core/build/fluxon-functions.json`）。 |
...
| `fluxonCompletion.trace` | `off` | 补全日志级别（`off`/`basic`/`verbose`）。 |
